...
# load SMNI EEG
data_smni_path = os.path.join(input_args.data_path, 'smni_eeg_processed.npz')
...
print(data.shape, np.unique(labels, return_counts=True))
tmp = numpy_tools.flatten_np(data)
if input_args.likelihood == 'bernoulli':
    tmp = data - data.min(axis=1, keepdims=True)
